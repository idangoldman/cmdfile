...
def find_cmd_file_paths(search_for="^cmd\.(yml|yaml|json|toml)$", folder="."):
    ...
    if 1 == len(cmd_files):
        ...
        print(
            "In this folder there is mroe than one CONFIG FILE. Folder Path:",
            folder_path,
        )
        sys.exit(1)
    ...
    if "/" != folder_path and str(pathlib.Path.home()) != folder_path:
        ...
        if parent_folders:
            found_paths.extend(parent_folders)
    ...
    with open(file_path, "r", encoding="utf-8") as content:
        ...
        try:
            if file_suffix in ["yml", "yaml"]:
                ...
                config = toml.loads(content.read())
...
def setup():
    ...
    cmd_file_default_path = os.path.join(
